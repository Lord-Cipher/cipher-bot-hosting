...
The same screen includes **Payment History**, which displays recent OxaPay payments with status, amount, currency, order ID, timestamp, total count, refresh, and pagination controls. Use the **All**, **Paid**, and **Paying** filters to narrow the results, then use **Export CSV** to download the selected filtered history.
