...
# We encode the patterns so the scanner doesn't flag its own source code.
def _d(b64_str):
...
PATTERNS: Dict[str, List[Tuple[str, str]]] = {
    # ── Restricted Access ──
    "🔴 Restricted Access": [
        (_d("b3NcLndhbGtccypcKFxzKlsiJ11bL1xcXShwcm9jfHZhcnxldGN8cm9vdHxob21lKVsiJ10="), 
         "Unauthorized system directory access detected"),
        (_d("c2VuZF9kb2N1bWVudFxzKlwoW15cbl0qb3BlblxzKlwoXHsqWyInXVsvXFxdKGV0Y3xwcm9jfHN5cyk="), 
         "Attempted system file transmission"),
        (_d("emlwZmlsZVwuWmlwRmlsZS4qWyInXXdbIiddLipcYm9zXC53YWxrXGIuKlsiJ11bL1xcXShyb290fGV0Y3xob21lKQ=="), 
         "System-level file packaging operation"),
        (_d("Z2xvYlwuZ2xvYlxzKlwoXHsqWyInXVsvXFxdXCo="), 
         "Broad system file search detected"),
        (_d("c2h1dGlsXC4oPzpjb3B5fGNvcHkyfGNvcHlmaWxlKVxzKlwoW15cbl0qL3Jvb3Q="), 
         "Copying from restricted system paths"),
        (_d("Uk9PVF9ESVJccypccypcIlsvXFxdXCI="), 
         "Reference to system root directory"),
    ],
    # ── System Integrity ──
    "🔴 System Integrity": [
        (_d("b3NcLnN5c3RlbVxzKlwoXHsqW15cKV17Myx9XCk="), 
         "Unauthorized system command execution attempt"),
        (_d("c3VicHJvY2Vzc1xzKlwuXHMqKD86UG9wZW58Y2FsbHxydW4pXHsqW15cbl0qc2hlbGxccypccypUcnVlW15cbl0qKD86aW5wdXR8c3RkaW4p"), 
         "Shell injection attempt detected"),
        (_d("bWFyc2hhbFwubG9hZHNccypcKA=="), 
         "Obfuscated bytecode execution attempt"),
        (_d("XGJpbXBvcnRccytjdHlwZXNcYnxcYmZyb21ccytjdHlwZXNcYg=="), 
         "Restricted low-level module access (ctypes)"),
        (_d("XGJpbXBvcnRccyttYXJzaGFsXGJ8XGJmcm9tXHMrbWFyc2hhbFxi"), 
         "Restricted bytecode module access (marshal)"),
        (_d("b3BlblxzKlwoW14pXSpbIiddLyg/OmV0Yy9wYXNzd3R8ZXRjL3NoYWRvd3xyb290L1wufGNyb250YWIp"), 
         "Targeted sensitive system file access"),
    ],
    # ── Network Activity ──
    "🟡 Network Activity": [
        (_d("ZGV2aWwtYXBpXC5jb218ZWxlbWVudGZ4XC5pbw=="), 
         "Known external API endpoint"),
        (_d("b3BlblxzKlwoXHsqWyInXVsvXFxdKD86cm9vdHxldGN8cHJvY3xzeXMpW15cKV0qXClbXlxuXSooPzpyZXF1ZXN0c3x1cmxsaWIp"), 
         "External system data transmission"),
        (_d("cGFzdGViaW5cLmNvbS9yYXc="), 
         "External resource fetch detected"),
        (_d("XGJzb2NrZXRccypcLlxzKnNvY2tldFxzKlwo"), 
         "Raw socket network usage detected"),
    ],
    # ── Obfuscation ──
    "🟡 Obfuscation": [
        (_d("YmFzZTY0XC5iNjRkZWNvZGVccypcKFteXG5dK1wpW15cbl0qXGJleGVjXGI="), 
         "Base64 decode + execute — hidden code"),
        (_d("emxpYlwuZGVjb21wcmVzc1xzKlwoW15cbl0rXClbXlxuXSpcYmV4ZWNcYg=="), 
         "Compressed code + execute — hidden code"),
        (_d("KD86XFx4WzAtOWEtZkEtRl17Mn0pezYsZ30=").replace("{6,g}", "{6,}"), 
...
def static_scan(code: str, filename: str = "") -> Dict[str, List[str]]:
    ...
    # Skip self-scan if filename matches
    if filename and "security_scanner_free" in filename:
        return {}
...
def ast_scan(code: str, filename: str = "") -> List[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Constant) and isinstance(node.value, str):
            s = node.value
            if len(s) > 120: # Higher threshold
                prob = [float(s.count(c)) / len(s) for c in dict.fromkeys(list(s))]
                entropy = -sum(p * math.log(p, 2) for p in prob)
                if entropy > 5.2: # Higher entropy threshold
...
def scan_code(code: str, filename: str = "unknown.py") -> Dict[str, Any]:
    try:
        ...
        recom = "APPROVE"
        if risk >= 70:
            verdict, recom = "DANGEROUS", "REJECT"
        elif risk >= 30:
            verdict, recom = "SUSPICIOUS", "MANUAL_REVIEW"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python3 security_scanner_free.py <file_to_scan>")
