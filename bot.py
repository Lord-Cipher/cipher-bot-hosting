...
def action_adm_pay_method_number(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("adm_pay_method_setnumber_"):
        ...
        bot.send_message(call.message.chat.id,
                         ...
                         f"{sc('Any text/number/address is accepted \u2014 send /cancel to abort')}.",
                         parse_mode="HTML")
        return
...
def render_adm_approval_group(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Enforcing', 'YES \u2014 users must join to use the bot' if grp_on else 'NO \u2014 not enforced yet, tap Verify below to turn on')}\n"
        f"{bullet('Groups configured', len(groups))}\n"
        f"{G['div']}\n"
        f"<b>{sc('Configured Groups')}:</b>\n{rows}\n"
        f"{G['div']}\n"
        f"<i>{sc('Note: adding a group here does NOT turn on enforcement by itself \u2014 you must also tap the Verify button below so it shows ON')}.</i>{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
